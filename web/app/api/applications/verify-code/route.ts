...
export async function POST(request: Request) {
  ...
  if (!portalApiUrl) {
    console.error("[verify-code] PORTAL_API_URL이 설정되지 않았습니다.");
  ...
  try {
    const res = await fetch(`${portalApiUrl}/api/inquiries/verify-code`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) {
      return NextResponse.json({ ok: false, errors: json.errors || ["인증번호 검증에 실패했습니다."] }, { status: res.status });
    }

    return NextResponse.json(json);
  } catch (error) {
    console.error("[verify-code] 포털 호출 실패", error);
